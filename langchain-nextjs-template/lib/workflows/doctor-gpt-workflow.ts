...
export class DoctorGPTWorkflow {
     ...
     * Gets responses from multiple AI providers using structured prompts
     */
    private async multiModelReasoningNode(state: DoctorGPTState): Promise<Partial<DoctorGPTState>> {
        ...
        try {
            ...
            const { PromptTemplateService } = await import('../preprocessing/prompt-templates');

            // Prepare retrieved chunks for prompt templates
            const retrievedChunks = this.prepareRetrievedChunks(state);

            // Create prompt context
            const promptContext = {
                userQuery: state.userQuery,
                retrievedChunks,
                patientContext: state.medicalContext ? {
                    age: state.medicalContext.patientAge,
                    gender: state.medicalContext.patientGender,
                    medicalHistory: state.medicalContext.medicalHistory,
                    currentSymptoms: state.medicalContext.currentSymptoms,
                    medications: state.medicalContext.medications,
                    allergies: state.medicalContext.allergies,
                    urgencyLevel: state.medicalContext.urgencyLevel as any
                } : undefined,
                sessionContext: {
                    sessionId: state.sessionId,
                    userId: state.userId,
                    uploadedDocuments: state.uploadedDocuments?.map(doc => doc.fileName) || []
                }
            };

            // Choose appropriate prompt based on query type and available documents
            let structuredPrompt;
            if (state.uploadedDocuments && state.uploadedDocuments.length > 0) {
                // Use document analysis prompt for uploaded documents
                structuredPrompt = PromptTemplateService.generateDocumentAnalysisPrompt(promptContext);
            } else {
                // Use medical consultation prompt for general queries
                structuredPrompt = PromptTemplateService.generateMedicalConsultationPrompt(promptContext);
            }

            console.log('Using structured prompt for multi-model reasoning');
            console.log('Prompt type:', state.uploadedDocuments?.length ? 'Document Analysis' : 'Medical Consultation');

            const enhancedSystemMessage = `${structuredPrompt.systemMessage}

${structuredPrompt.expectedFormat}

${structuredPrompt.citationInstructions}

${structuredPrompt.reasoningInstructions}`;

            const multiModelResult = await modelRepository.multiModelReasoning(
                [
                    { role: 'system', content: enhancedSystemMessage },
                    { role: 'user', content: structuredPrompt.userMessage }
                ],
                {
                    temperature: 0.7,
                    maxTokens: 2000
            ...
            // Enhance the final response with structured prompt metadata
            const enhancedFinalResponse = {
                ...multiModelResult.finalResponse,
                metadata: {
                    ...multiModelResult.finalResponse.metadata,
                    promptType: state.uploadedDocuments?.length ? 'document_analysis' : 'medical_consultation',
                    structuredPrompt: true,
                    citationRequired: true,
                    stepwiseReasoning: true
                }
            };
            ...
            return {
                ...
                metadata: {
                    ...
                    promptingStrategy: 'structured_medical_prompts',
                    documentsProcessed: retrievedChunks.length
                }
            };
    ...
    private prepareModelContext(state: DoctorGPTState): string {
        ...
        if (state.retrievedDocuments?.length) {
            ...
            state.retrievedDocuments.forEach((doc: any, i) => {
     ...
     * Prepare retrieved chunks for prompt templates
     */
    private prepareRetrievedChunks(state: DoctorGPTState): Array<{
        id: string;
        content: string;
        source: string;
        ...
        // Add retrieved documents (from vector search or uploaded documents)
        if (state.retrievedDocuments?.length) {
            state.retrievedDocuments.forEach((doc: any, index) => {
                chunks.push({
                    id: doc.id || `retrieved-${index}`,
                    content: doc.content || 'No content available',
                    source: doc.source || 'vector_database',
                    relevanceScore: doc.relevanceScore || 0.8,
                    metadata: {
                        fileName: doc.fileName,
                        docType: doc.metadata?.fileType || doc.metadata?.reportType,
        ...
        if (state.searchResults?.length) {
            state.searchResults.forEach((result: any, index) => {
                chunks.push({
                    id: `search-${index}`,
        ...
        // If no documents retrieved but we have uploaded documents in state, use those
        if (chunks.length === 0 && state.uploadedDocuments?.length) {
            state.uploadedDocuments.forEach((doc: any, index) => {
                chunks.push({
                    id: doc.id || `uploaded-${index}`,
                    content: doc.content || doc.extractedText || 'No content available',
                    source: 'uploaded',
                    relevanceScore: 1.0, // High relevance for explicitly uploaded documents
                    metadata: {
                        fileName: doc.fileName,
                        fileType: doc.fileType,
                        docType: doc.reportType || 'unknown',
                        documentId: doc.id,
